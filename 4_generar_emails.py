...
"""NGLAB Motor — Paso 4: Generar Email 1 por plantilla.

Email 1 — Día 0:
- CTA principal: enlace al informe personalizado (tracking real por clic)
- CTA secundario: WhatsApp
- Pain points como gancho corto, NO revelados en detalle (eso lo hace el informe)
- Sin coste de API. Sin dependencia de Anthropic.
"""
...
def generar_email(lead: dict) -> dict | None:
    ...
    # Construir 3 puntos de gancho — CORTOS, no revelamos todo (eso lo hace el informe)
    puntos = []

    if rating and resenas:
        rating_f = float(rating)
        if rating_f >= 4.5 and resenas < 50:
            puntos.append(f"Tenéis un {rating}/5 con {resenas} reseñas — buena base, pero aparecéis por debajo de competidores con más visibilidad")
        elif rating_f >= 4.0:
            puntos.append(f"Con {resenas} reseñas y {rating}/5 en Google tenéis credibilidad, pero hay cosas que os están frenando")
        elif rating_f < 4.0:
            puntos.append(f"Vuestro {rating}/5 en Google con {resenas} reseñas está por debajo de la media del sector")
    elif tiene_web:
        puntos.append(dolor_cfg or f"Los clientes buscan {sector_label} en Google antes de llamar")
    else:
        puntos.append(f"Sin web propia solo existís en Google Maps — perdéis clientes cada día frente a competidores con presencia digital")

    if tiene_web:
        if necesita_citas and not tiene_citas_web:
            puntos.append(f"El 30% de las búsquedas de {sector_label} ocurren fuera del horario comercial")
        elif not tiene_whatsapp:
            puntos.append(f"Los clientes prefieren escribir por WhatsApp antes que llamar")
        else:
            puntos.append(f"El 46% de las búsquedas en Google tienen intención local — cada día sin visibilidad son clientes perdidos")
    else:
        puntos.append(f"El 88% de quien busca un negocio local lo visita en 24h — sin web no llegáis a esa ventana")

    puntos.append(f"Cuando alguien le pregunta a ChatGPT o Google 'mejor {sector_label} en {ciudad}', {nombre} no aparece")
    ...
    # ── CUERPO TEXTO PLANO ────────────────────────────────────────────────
    if tiene_web:
        intro_texto = f"He analizado la presencia digital de {nombre} y hay cosas que creo que os interesan. Os he preparado un informe gratuito:"
    else:
        intro_texto = f"He buscado {nombre} online y hay cosas que creo que os interesan. Os he preparado un informe gratuito:"
...
Os he preparado un análisis completo con los detalles y cómo mejorar cada punto:

Ver vuestro informe gratuito →
{url_informe}

Si preferís hablar directamente, estoy en WhatsApp:
{wa_url}
...
"""
    ...
    if tiene_web:
        intro_html = f"He analizado la presencia digital de <strong>{nombre}</strong> {rating_badge} y hay cosas que os interesan:"
    else:
        intro_html = f"He buscado <strong>{nombre}</strong> online {rating_badge} y hay cosas que os interesan:"
...
<td width="1" style="padding:0 12px"><div style="width:1px;height:36px;background:#d1fae5"></div></td>
...
<td style="padding-left:14px">
  <p style="margin:0;font-size:12px;color:#065f46;line-height:1.5">Tenéis buena reputación — pero hay puntos que os están costando clientes. Los detallamos en vuestro informe.</p>
</td>
</tr>
</table>
</td>
...
<p style="margin:0 0 18px;color:#1a1a1a;font-size:14px;line-height:1.7">
  Soy <strong>Jesica</strong> de N&amp;G LAB Digital. {intro_html}
</p>
...
<td style="padding-left:12px;background:#fafafa;border-radius:8px;padding:12px 14px;border:1px solid #f0f0f0">
...
<td style="padding-left:12px;background:#fffbeb;border-radius:8px;padding:12px 14px;border:1px solid #fde68a">
...
<!-- CTA PRINCIPAL — Informe -->
<table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:14px">
<tr>
<td style="background:#14141A;border-radius:8px;padding:22px 20px;text-align:center">
  <p style="margin:0 0 4px;color:#C8FF00;font-size:11px;font-weight:700;letter-spacing:1px">VUESTRO INFORME GRATUITO</p>
  <p style="margin:0 0 16px;color:#888;font-size:12px;line-height:1.5">Análisis completo con los problemas detectados,<br>cómo resolverlos y presupuesto orientativo</p>
  <a href="{url_informe}"
     style="background:#C8FF00;color:#14141A;padding:13px 32px;border-radius:6px;font-weight:800;font-size:15px;text-decoration:none;display:inline-block;letter-spacing:0.3px">
    Ver mi análisis gratuito →
  </a>
  <p style="margin:12px 0 0;color:#555;font-size:11px">Tarda 30 segundos · Sin registro · Sin compromiso</p>
</td>
</tr>
</table>
...
<td style="text-align:center;padding:6px 0">
  <a href="{wa_url}"
     style="color:#25D366;font-family:system-ui,sans-serif;font-size:13px;font-weight:600;text-decoration:none">
